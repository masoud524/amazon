...
try {
    ...
    while (true) {
        ...
        $data['product_names'] = array_merge($data['product_names'], $matches[1]);

        // Extract product URLs
        $pattern = '/s-no-outline\" href=\"([^\"]+)/i';
        preg_match_all($pattern, $html, $urls);
        $data['product_urls'] = array_merge($data['product_urls'], $urls[1]);
    ...
    file_put_contents('web/'.$_REQUEST["name"].'.json', json_encode($data));
