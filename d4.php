...
try {
    ...
    $page->setUserAgent($_REQUEST["name"]);

    // Load JSON data from file
    $jsonContent = file_get_contents('web/'.$_REQUEST["name"].'.json');
    $decodedData = json_decode($jsonContent, true); // Decode JSON as associative array
    
    //{چک کردن وجود پوشه}
    if(!is_dir('data/'.$_REQUEST["name"])){
     mkdir('data/'.$_REQUEST["name"]);
    }

    if ($decodedData && isset($decodedData["product_urls"])) {
        // Loop through a limited number of URLs (e.g., first 10)
        for ($i = $_REQUEST["az"]; $i < $_REQUEST["ta"] && $i < count($decodedData["product_urls"]); $i++) {
            ...
            file_put_contents('data/'.$_REQUEST["name"].'/'.$_REQUEST["name"].$i.'.html', $html);
        ...
        file_put_contents('data/'.$_REQUEST["name"].'/'.$_REQUEST["name"].'.json', json_encode($data));
    } else {
